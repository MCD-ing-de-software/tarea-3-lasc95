...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(df, ["name", "does_not_exist"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        result = cleaner.trim_strings(df_to_clean, ["name"])

        # 1. Verify original DataFrame was not modified
        pdt.assert_frame_equal(df_to_clean, original_df)
        
        # 2. Verify the result is as expected
        expected_df = original_df.copy()
        expected_df["name"] = ["Alice", "Bob", None, "Carol"]
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        cleaner = DataCleaner()
        # The data from make_sample_df() is not enough to make 120 an outlier.
        # We create a specific DataFrame for this test case.
        df = pd.DataFrame({'age': [25, 30, 32, 35, 120]})
